...
const CardList = ({ item, onClick, onDeleteClick, basket = [] }) => {
  ...
  return (
    <div className={`h-[60px] w-full bg-gray-200 relative flex rounded-lg fadeInHalfSecondCards`}>
      <div
        className={`w-full transition-all duration-200 group ease-linear  hover:bg-gray-400 cursor-pointer flex        border-2 border-gray-700 ${
          isItemInBasket
        ...
        <div className="flex gap-[20px] w-full py-4 px-10  transition-all duration-200 ease-linear items-center">
          ...
          <div className="flex items-center gap-[25px]">
            ...
            <p className="text-3xl font-bold text-right my-auto w-[120px]">${item.price}</p>
          </div>
        </div>
        {isItemInBasket && (
          <div
        ...
        ease-in cursor-not-allowed text-2xl hidden group-hover:flex">
            <p className="w-[500px] text-center bg-gray-400 p-3 rounded-lg">
